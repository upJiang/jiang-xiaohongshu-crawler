...
async function analyzeWithAI(dataList) {
    ...
    try {
        // 构建批量分析的提示词
        const contentToAnalyze = dataList.map((data, index) => 
            `内容${index + 1}:
            标题：${data.笔记标题}
            内容：${data.笔记内容}`
        ).join('\n\n');
        ...
        // 将返回的文本按内容分块
        const contentBlocks = analysisText.split(/内容\d+:/g).filter(block => block.trim());
        
        // 解析每个内容块
        return contentBlocks.map(block => {
            const sentimentMatch = block.match(/情感倾向：(正面|负面)/);
            const reasoningMatch = block.match(/分析理由：(.+?)(?=(?:\n\n|\n内容\d+:|$))/s);
...
async function getExistingLinks() {
    try {
        const filePath = path.join(__dirname, '小红书爬虫文件', '最新舆情数据.xlsx');
...
async function main() {
    ...
    // 设置关键词
    const keyword = '研选家';
    await page.goto(`https://www.xiaohongshu.com/search_result?keyword=${keyword}&source=web_search_result_notes&type=51`);
    ...
    while (true) {
        ...
        for (const element of divList) {
            ...
            if (href) {
                const fullHref = 'https://www.xiaohongshu.com/explore/' + href.split('/')[2];
                // 检查是否已存在该链接，包括Excel中的历史数据
                if (!uniqueHrefs.includes(fullHref) && !existingLinks.includes(fullHref)) {
    ...
    const keywords = XLSX.utils.sheet_to_json(keywordWorkbook.Sheets[keywordWorkbook.SheetNames[0]], { header: 1 });
    ...
    for (const link of uniqueHrefs) {
        ...
        try {
            ...
            const data = await page.evaluate(() => {
                ...
                const noteContent = content ? content.textContent.trim() : '';
                ...
                if (commentSection) {
                    ...
                    commentContent = noteText ? noteText.textContent.trim() : '';
                }

                return {
                    '笔记标题': getTextContent('.title'),
                    '笔记作者': getTextContent('.username'),
                    '笔记时间': getTextContent('.date'),
